...
export function ClanLeaderboard({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(data);
      try {
        setMemberIds(await fetchClanMemberIds(data.map((r) => r.id)));
      } catch {
        setMemberIds({});
      }
    } catch (e) {
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.list}>
          {rows.map((r) => {
            ...
            const onlineCount = (memberIds[r.id] ?? []).filter((id) => onlineIds.has(id)).length;
            return (
              <View key={r.id} style={[styles.row, mine && styles.rowMine]}>
                ...
                <View style={styles.info}>
                  ...
                  <View style={styles.membersRow}>
                    ...
                    <Text style={styles.members}>{onlineCount} çevrimiçi</Text>
                  </View>
                </View>
